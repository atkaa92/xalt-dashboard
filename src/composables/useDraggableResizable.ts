...
import type { OverlayStyle, ContainerDimensions } from '../utilities/types'; // Adjust path if necessary

export function useDraggableResizable(
    initialStyle: OverlayStyle,
    containerDimensions: ContainerDimensions,
    isImage: Ref<boolean> | boolean // Accept Ref<boolean> or boolean
) {
    const isDragging = ref(false);
    const isResizing = ref(false);

    // Store necessary initial values during interaction
    let startX: number, startY: number;
    let initialLeft: number, initialTop: number, initialWidth: number, initialHeight: number;
    let activeResizeHandle: string = '';

    // The reactive style object that will be bound to the element
    const style: OverlayStyle = reactive<OverlayStyle>({ ...initialStyle });
    const minSizePercent = 5; // Minimum 5% size

    const updateStyle = (updates: Partial<OverlayStyle>) => {
        Object.assign(style, updates);
    };

    // Helper to get boolean value from Ref or static value
    ...
    const onResize = (event: MouseEvent | TouchEvent) => {
        ...
        if (handle.includes('r')) { newWidthPx = initialWidth + deltaX; }
        if (handle.includes('l')) { newWidthPx = initialWidth - deltaX; }
        if (handle.includes('b') && !isCurrentImg) { newHeightPx = initialHeight + deltaY; }
        if (handle.includes('t') && !isCurrentImg) { newHeightPx = initialHeight - deltaY; }
        ...
        updateStyle({
            ...
            height: isCurrentImg ? 'auto' : `${(newHeightPx / containerDimensions.height * 100).toFixed(2)}%`,
        });
    };
